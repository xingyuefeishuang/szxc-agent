...
-- ----------------------------
-- Table structure for o_fulfillment_voucher (实际码券表)
-- ----------------------------
...
CREATE TABLE `o_refund_apply` (
  ...
  `order_item_id` bigint(20) DEFAULT NULL COMMENT '关联的具体子单(可选)，暂不支持',
...
CREATE TABLE `o_channel_sku_mapping` (
  ...
  `channel_merchant_id` varchar(64) DEFAULT NULL COMMENT '渠道侧商家ID（如抖音商家ID）',
  `spu_id` varchar(128) NOT NULL COMMENT '强制锚定到系统内部的底层SPU',
  `sku_id` varchar(128) NOT NULL DEFAULT '0' COMMENT '精细描绘到内部具体SKU(无SKU则赋0)',
  `merchant_id` varchar(64) DEFAULT NULL COMMENT '平台内部商户ID',
...
-- ----------------------------
-- Table structure for o_channel_shop_mapping (渠道门店与平台店铺映射表)
-- ----------------------------
DROP TABLE IF EXISTS `o_channel_shop_mapping`;
CREATE TABLE `o_channel_shop_mapping` (
  `mapping_id`          bigint(20)   NOT NULL COMMENT '主键(雪花算法)',
  `channel_code`        varchar(32)  NOT NULL COMMENT '渠道编码（如 DOUYIN / MEITUAN）',
  `channel_merchant_id` varchar(64)  NOT NULL COMMENT '渠道侧商家ID（如抖音商家ID）',
  `channel_shop_id`     varchar(64)  NOT NULL DEFAULT '' COMMENT '渠道侧门店ID（如抖音poi_id，无门店层级则留空）',
  `channel_shop_name`   varchar(128) DEFAULT NULL COMMENT '渠道侧门店名称（冗余，便于排查）',
  `merchant_id`         varchar(64)  NOT NULL COMMENT '平台内部商户ID',
  `shop_id`             varchar(64)  NOT NULL DEFAULT '' COMMENT '平台内部店铺ID（无店铺层级则留空）',
  `prod_id`             varchar(64)  NOT NULL DEFAULT '0' COMMENT '产品线ID',
  `app_id`              varchar(64)  NOT NULL DEFAULT '0' COMMENT '应用模块ID',
  `status`              tinyint(1)   NOT NULL DEFAULT '1' COMMENT '状态：1启用 0禁用',
  `tenant_id`           bigint(20)   NOT NULL DEFAULT '0' COMMENT '多租户ID',
  `create_user`         bigint(20)   NOT NULL DEFAULT '0' COMMENT '创建者',
  `modify_user`         bigint(20)   NOT NULL DEFAULT '0' COMMENT '修改者',
  `create_time`         datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `modify_time`         datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted`             tinyint(1)   NOT NULL DEFAULT '0' COMMENT '逻辑删除',
  PRIMARY KEY (`mapping_id`),
  UNIQUE KEY `uk_chan_shop` (`channel_code`, `channel_merchant_id`, `channel_shop_id`),
  KEY `idx_merchant_shop` (`merchant_id`, `shop_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='渠道门店与平台店铺映射表';
...
CREATE TABLE `o_fulfillment_rule` (
  ...
  `scope_type` varchar(32) NOT NULL COMMENT '作用域类型(PROD/APP/SPU/SKU)',
  `scope_id` varchar(128) NOT NULL COMMENT '作用域ID',
  `fulfillment_type` varchar(32) NOT NULL DEFAULT 'VOUCHER' COMMENT '履约类型(VOUCHER发码,OFFLINE线下履约)',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='履约规则配置表 (废弃)';
